...
def build_layout(asset_images: dict[str, Image.Image]) -> Image.Image:
    ...
    draw.rectangle((0, 0, 7000, 14), fill=YELLOW)
    draw.rectangle((0, 0, 18, 1000), fill=YELLOW)

    place(canvas, asset_images["logo-completa"], (120, 35, 2190, 555))
    place(canvas, asset_images["whatsapp-icone"], (120, 610, 540, 955))
    place(canvas, asset_images["telefone-01"], (590, 595, 2200, 755))
    place(canvas, asset_images["telefone-02"], (590, 770, 2200, 930))
    ...
    card_y1, card_y2 = 330, 790
    card_width = 1030
    starts = [2500, 3610, 4720, 5830]
    specialty_names = [
    ...
    for index, (icon_name, text_name) in enumerate(specialty_names):
        x1 = starts[index]
        x2 = x1 + card_width
        rounded_card(draw, (x1, card_y1, x2, card_y2), primary=index == 0)
        place(canvas, asset_images[icon_name], (x1 + 45, card_y1 + 65, x1 + 430, card_y2 - 65))
        place(canvas, asset_images[text_name], (x1 + 455, card_y1 + 65, x2 - 35, card_y2 - 65))

    draw.rectangle((2480, 825, 6930, 995), fill=(5, 7, 8, 235))
...
def main() -> None:
    ...
    layout_jpg = LAYOUTS / "ckf-banner-7x1-recortes-originais-recomendado.jpg"
